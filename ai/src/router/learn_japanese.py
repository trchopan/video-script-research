...
class PostLearnJapaneseProcessText(BaseModel):
    text: str


@learn_japanese_router.post("/learn_japanese_process_text")
def post_learn_japanese_process_text(body: PostLearnJapaneseProcessText):
    result = learn_japanese_svc.process_text(body.text)
    return {"texts": [r.dict() for r in result]}
